...
AXIS = dict(
    ...
    ticks='outside', tickfont=dict(size=12, color='black'),
...
with col1:
    ...
    if not df_sm.empty:
        ...
        fig.add_trace(go.Scatter(
            x=df_sm['readingId'], y=df_sm['value'],
            mode='lines+markers', name='Soil Moisture',
            line=dict(color='#4caf50', width=2),
            marker=dict(size=8, color='#4caf50'), showlegend=True
        ))
        anom = df_sm[df_sm['anomalyFlag'] == 1]
        if not anom.empty:
            fig.add_trace(go.Scatter(
                x=anom['readingId'], y=anom['value'],
                mode='markers', name='⚠ Anomaly',
                marker=dict(color='red', size=14, symbol='x', line=dict(width=3)), showlegend=True
            ))
        fig.add_hline(y=18, line_dash='dash', line_color='orange',
                      annotation_text='Min threshold 18%',
                      annotation_font=dict(color='orange', size=11))
        fig.update_layout(
            **LAYOUT,
            xaxis=dict(title='Reading #', **AXIS),
            yaxis=dict(title='Soil Moisture (%)', **AXIS),
            legend=dict(orientation='h', y=-0.30, font=dict(size=13, color='black'), bgcolor='white', bordercolor='black', borderwidth=1)
        )
...
with col2:
    ...
    if not df_temp.empty:
        ...
        fig2.add_trace(go.Scatter(
            x=df_temp['readingId'], y=df_temp['value'],
            mode='lines+markers', name='Temperature',
            line=dict(color='#ff9800', width=2),
            marker=dict(size=8, color='#ff9800'), showlegend=True
        ))
        anom_t = df_temp[df_temp['anomalyFlag'] == 1]
        if not anom_t.empty:
            fig2.add_trace(go.Scatter(
                x=anom_t['readingId'], y=anom_t['value'],
                mode='markers', name='⚠ Anomaly',
                marker=dict(color='red', size=14, symbol='x', line=dict(width=3)), showlegend=True
            ))
        ...
        fig2.update_layout(
            **LAYOUT,
            xaxis=dict(title='Reading #', **AXIS),
            yaxis=dict(title='Temperature (°C)', **AXIS),
            legend=dict(orientation='h', y=-0.30, font=dict(size=13, color='black'), bgcolor='white', bordercolor='black', borderwidth=1)
        )
...
# ── Irrigation Events ────────────────────────────────────────────
with col3:
    st.markdown("##### 🚿 Irrigation Events — Water Volume (m³)")
    df_irr = run_query("""
    ...
    if not df_irr.empty:
        fig3 = px.bar(
            df_irr, x='irrigStartTime', y='waterVolume_m3',
        ...
        fig3.update_layout(
            **LAYOUT,
            xaxis=dict(title='Date', **AXIS),
            yaxis=dict(title='Water Volume (m³)', **AXIS),
            legend=dict(orientation='h', y=-0.35, font=dict(size=13, color='black'), bgcolor='white', bordercolor='black', borderwidth=1)
        )
        fig3.update_layout(margin=dict(t=30, b=80, l=60, r=20))
...
with col4:
    ...
    if not df_yield.empty:
        ...
        fig4.update_layout(
            **LAYOUT,
            xaxis=dict(title='Crop', **AXIS),
            yaxis=dict(title='Total Yield (t)', **AXIS),
            showlegend=False
        )
...
with col5:
    ...
    if not df_q1.empty:
        fig_q1 = px.bar(
            df_q1, x='Crop', y='Yield/ha (t/ha)', color='Crop',
            color_discrete_sequence=['#2e7d32','#66bb6a','#a5d6a7','#1b5e20'],
            text='Yield/ha (t/ha)'
        )
        fig_q1.update_traces(textposition='outside', textfont=dict(size=12, color='black'))
        fig_q1.update_layout(
            **LAYOUT,
            xaxis=dict(title='Crop', **AXIS),
            yaxis=dict(title='Yield per Hectare (t/ha)', **AXIS),
            showlegend=False
        )
...
with col6:
    ...
    if not df_q2.empty:
        fig_q2 = px.bar(
            df_q2, x='Field', y='Anomaly Rate (%)', color='Field',
            color_discrete_sequence=px.colors.qualitative.Set2,
            text='Anomaly Rate (%)'
        )
        fig_q2.update_traces(textposition='outside', textfont=dict(size=12, color='black'))
        fig_q2.update_layout(
            **LAYOUT,
            xaxis=dict(title='Field', **AXIS),
            yaxis=dict(title='Anomaly Rate (%)', **AXIS),
            showlegend=False
        )
